...
for start in starts:
    ...
    lo = circles[t]+'c'
    if t % 2 == 0:
        newpawn = canvas.create_oval(b1,b2,b3,b4, outline = w, fill = w, tags = ('w', lo))
...
def click(event):
	...
	if opp in canvas.gettags(canvas.find_withtag(str(checking)+'c')): #oposite color?
		...
		scores.append(checking - pattern[options])
		...
		newpawn = canvas.create_oval(b1,b2,b3,b4, fill = f, outline = f,
				     tags = (same,  (str(pos)+'c')))
		print(canvas.gettags(newpawn))
		for scored in range(len(scores)):
		    change = str(scores[scored])+'c'
